...
from core.intent_summary import IntentSummaryService
...
def test_build_intent_summary_groups_same_value_different_fields_with_or():
    ...
    conditions = [
        ...
        Condition(field="policies_product_name", operator=Operator.NESTED_MATCH, value="e生保"),
    ]
...
def test_intent_summary_loads_unsupported_fields_from_enhanced_rules(tmp_path):
    field_definitions_path = tmp_path / "field_definitions.yaml"
    field_definitions_path.write_text("intents: []\n", encoding="utf-8")
    ...
    service = IntentSummaryService(
        field_definitions_path=str(field_definitions_path),
        enhanced_rules_path=str(enhanced_rules_path),
    ).load()
