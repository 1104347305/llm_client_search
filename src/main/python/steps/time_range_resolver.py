...
from src.main.python.models.schemas import RangeValue
...
def resolve_dynamic_date_range(config: Dict, match=None, now: Optional[Union[date, datetime]] = None):
    ...
    if date_range == "month_offset":
        ...
        while month < 1:
            ...
            year -= 1
        last_day = calendar.monthrange(year, month)[1]
        return _range(date(year, month, 1), date(year, month, last_day))

    if date_range == "past_month_to_today":
        # 上月1号 ~ 今天（近一个月语义，不受当月天数影响）
        year = today.year
        month = today.month - 1
        if month < 1:
            month = 12
    ...
    if date_range == "years_ago_today":
        ...
        years = int(config.get("years", 0))
        years_group = config.get("years_group")
        if years_group and match:
            try:
                years = int(match.group(years_group))
            except (IndexError, ValueError):
                pass
...
def resolve_dynamic_date_placeholder(value, now: Optional[datetime] = None):
    ...
    exact_mapping = {
        ...
        "<next_7_days_start>": _format_date_value(today, "YYYY-MM-DD HH:mm:ss"),
        "<next_7_days_end>": _format_date_value(today + timedelta(days=6), "YYYY-MM-DD HH:mm:ss"),
        "<next_30_days_start>": _format_date_value(today, "YYYY-MM-DD HH:mm:ss"),
        "<next_30_days_end>": _format_date_value(today + timedelta(days=29), "YYYY-MM-DD HH:mm:ss"),
    }
    ...
    minus_days = re.fullmatch(r"<today(?:-|_minus_)(\d+)_?days>", normalized, re.IGNORECASE)
    if minus_days:
        target = today - timedelta(days=int(minus_days.group(1)))
        return _format_date_value(target, "YYYY-MM-DD HH:mm:ss")
