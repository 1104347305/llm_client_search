...
try:
    import redis
except ImportError:  # pragma: no cover - 本地未安装 redis 客户端时降级禁用 L3
    redis = None
from src.main.python.config.settings import settings
from src.main.python.models.schemas import ParsedQuery, Condition, QueryLogic, Sort, SortOrder, Operator, RangeValue
...
class Level3SemanticCache:
    ...
    def __init__(self):
        """初始化缓存"""
        if redis is None:
            logger.warning("Redis package is not installed, semantic cache disabled")
