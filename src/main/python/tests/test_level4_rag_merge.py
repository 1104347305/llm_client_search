...
class _StubFieldRegistry:
    ...
    def format_prompt_section(self, intents, query="", max_chars=0):
...
def test_level4_rag_keeps_same_field_with_different_operators(monkeypatch):
    ...
    class _OperatorAwareRegistry(_StubFieldRegistry):
        def retrieve(self, query, top_k=10):
            return [
                ...
                {"id": "es_not_contains", "field": "planAbbrNames", "operator": "NOT_CONTAINS", "value_type": "enum", "examples": []},
    ...
    assert "planAbbrNames:NOT_CONTAINS" in message
    assert "planAbbrNames:NOT_EXISTS" in message
