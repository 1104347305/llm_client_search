...
def test_a_customer_pattern_supports_mid_high_temperature_group():
    matcher = Level2EnhancedMatcher("config/enhanced_rules.yaml")

    conditions = asyncio.run(matcher.match("A类中高温客户"))

    assert len(conditions) == 2
    values = {(cond.field, cond.operator.value): cond.value for cond in conditions}
    assert values[("clientTemperature", "CONTAINS")] == ["中温", "高温"]
    assert values[("newValueLabel", "CONTAINS")] == ["A1", "A2", "A3", "A4"]


def test_mid_high_temperature_group_supports_reverse_order_with_a_customer():
    matcher = Level2EnhancedMatcher("config/enhanced_rules.yaml")

    conditions = asyncio.run(matcher.match("中高温A类客户"))
