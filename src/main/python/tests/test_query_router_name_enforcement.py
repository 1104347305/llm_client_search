...
try:
    ...
    from models.schemas import Condition, Operator, ParsedQuery, QueryLogic, RangeValue
    from utils.name_candidate import detect_name_candidate
except ModuleNotFoundError:
    from src.main.python.steps.query_router import QueryRouter
    from src.main.python.models.schemas import Condition, Operator, ParsedQuery, QueryLogic, RangeValue
    from src.main.python.utils.name_candidate import detect_name_candidate
...
class _StubLevel4AgeBounds:
    async def parse(self, query):
        return ParsedQuery(
            conditions=[
                Condition(field="clientAge", operator=Operator.GTE, value=30),
                Condition(field="clientAge", operator=Operator.LTE, value=60),
                Condition(field="mariSts", operator=Operator.MATCH, value="未婚"),
                Condition(field="clientTemperature", operator=Operator.MATCH, value="高温"),
            ],
            query_logic=QueryLogic.AND,
            confidence=0.8,
            matched_level=4,
...
def test_route_with_peeling_merges_l4_gte_lte_bounds():
    router = _build_router(_StubLevel4AgeBounds())
    router._valid_fields = {"clientAge", "mariSts", "clientTemperature"}

    parsed = asyncio.run(router.route_with_peeling("30到60岁未婚高温客户"))

    assert len(parsed.conditions) == 3
    age_condition = parsed.conditions[0]
    assert age_condition.field == "clientAge"
    assert age_condition.operator == Operator.RANGE
    assert age_condition.value == RangeValue(min=30, max=60)
    assert [(cond.field, cond.operator, cond.value) for cond in parsed.conditions[1:]] == [
        ("mariSts", Operator.MATCH, "未婚"),
        ("clientTemperature", Operator.MATCH, "高温"),
    ]
