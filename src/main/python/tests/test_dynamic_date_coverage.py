...
from src.main.python.steps.time_range_resolver import resolve_dynamic_date_range, resolve_dynamic_date_placeholder
...
def test_level2_matches_birthday_recent_window():
    ...
    assert condition.value.min == date.today().strftime("%m-%d")
...
def test_level2_matches_policy_recent_expiry():
    ...
    assert condition.operator.value == "RANGE"
    assert condition.value.min == date.today().strftime("%Y-%m-%d")
    assert condition.value.max == (date.today() + timedelta(days=29)).strftime("%Y-%m-%d")


def test_level2_matches_annual_premium_small_plain_number():
    matcher = Level2EnhancedMatcher()

    conditions = asyncio.run(matcher.match("年交保费大于1"))

    assert len(conditions) == 1
    condition = conditions[0]
    assert condition.field == "annPremSegNum"
    assert condition.operator.value == "GTE"
    assert condition.value == 1


def test_level2_matches_prospect_source_short_name():
    matcher = Level2EnhancedMatcher()

    conditions = asyncio.run(matcher.match("准客来源意健险"))

    assert len(conditions) == 1
    condition = conditions[0]
    assert condition.field == "pcustSourcType"
    assert condition.operator.value == "MATCH"
    assert condition.value == "意健险准客"
...
def test_level2_matches_recent_underwriting_date():
    ...
    assert len(conditions) == 1
    condition = conditions[0]
    assert condition.field == "latelyUndwrtSegTime"
    assert condition.operator.value == "RANGE"
...
def test_unified_relative_time_ranges_are_stable():
    ...
    assert next_week_window.min == "2026-03-25 00:00:00"
    assert next_week_window.max == "2026-03-31 00:00:00"
...
def test_level4_resolves_new_relative_placeholders():
    ...
    assert resolve_dynamic_date_placeholder("<next_7_days_start>", now=base_now) == "2026-03-25 00:00:00"
    assert resolve_dynamic_date_placeholder("<next_7_days_end>", now=base_now) == "2026-03-31 00:00:00"
    assert resolve_dynamic_date_placeholder("<next_30_days_start>", now=base_now) == "2026-03-25 00:00:00"
    assert resolve_dynamic_date_placeholder("<next_30_days_end>", now=base_now) == "2026-04-23 00:00:00"
    assert resolve_dynamic_date_placeholder("<today_minus_180_days>", now=base_now) == "2025-09-26 00:00:00"
